...
_SUCCESS_RE = re.compile('^-- (Check|Looking|Performing (.*)?Test|Detecting).*-{1,2} (works|done|yes|found|[Ss]uccess)$')
_SUCCESS2_RE = re.compile('^-- Found .*:\s.*$')
_FAILURE_RE = re.compile('^-- .* -{1,2} (no|not found|[Ff]ailed|NOTFOUND)$')
